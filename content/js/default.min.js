...
(function(a) {
 ...
 tb.duration.fn.asMilliseconds = function() {
  ...
  function(a) {
   ...
   function c(a, c, d) {
    var e = {
     mm: c ? "Ñ…Ð²Ñ–Ð»Ñ–Ð½Ð°_Ñ…Ð²Ñ–Ð»Ñ–Ð½Ñ‹_Ñ…Ð²Ñ–Ð»Ñ–Ð½" : "Ñ…Ð²Ñ–Ð»Ñ–Ð½Ñƒ_Ñ…Ð²Ñ–Ð»Ñ–Ð½Ñ‹_Ñ…Ð²Ñ–Ð»Ñ–Ð½",
     hh: c ? "Ð³Ð°Ð´Ð·Ñ–Ð½Ð°_Ð³Ð°Ð´Ð·Ñ–Ð½Ñ‹_Ð³Ð°Ð´Ð·Ñ–Ð½" : "Ð³Ð°Ð´Ð·Ñ–Ð½Ñƒ_Ð³Ð°Ð´Ð·Ñ–Ð½Ñ‹_Ð³Ð°Ð´Ð·Ñ–Ð½",
     dd: "Ð´Ð·ÐµÐ½ÑŒ_Ð´Ð½Ñ–_Ð´Ð·Ñ‘Ð½",
     MM: "Ð¼ÐµÑÑÑ†_Ð¼ÐµÑÑÑ†Ñ‹_Ð¼ÐµÑÑÑ†Ð°Ñž",
     yy: "Ð³Ð¾Ð´_Ð³Ð°Ð´Ñ‹_Ð³Ð°Ð´Ð¾Ñž"
    };
    return "m" === d ? c ? "Ñ…Ð²Ñ–Ð»Ñ–Ð½Ð°" : "Ñ…Ð²Ñ–Ð»Ñ–Ð½Ñƒ" : "h" === d ? c ? "Ð³Ð°Ð´Ð·Ñ–Ð½Ð°" : "Ð³Ð°Ð´Ð·Ñ–Ð½Ñƒ" : a + " " + b(e[d], +a)
   }
  ...
  function(a) {
   ...
   var h = {
    ...
    i = "sekmadienis_pirmadienis_antradienis_treÄiadienis_ketvirtadienis_penktadienis_Å¡eÅ¡tadienis".split("_");
...
! function(a) {
 ...
 a.extend(a.validator.messages, {
  ...
  maxlength: a.validator.format("LÃ¼tfen en fazla {0} karakter uzunluÄŸunda bir deÄŸer giriniz."),
  minlength: a.validator.format("LÃ¼tfen en az {0} karakter uzunluÄŸunda bir deÄŸer giriniz."),
  rangelength: a.validator.format("LÃ¼tfen en az {0} ve en fazla {1} uzunluÄŸunda bir deÄŸer giriniz."),
  range: a.validator.format("LÃ¼tfen {0} ile {1} arasÄ±nda bir deÄŸer giriniz."),
  max: a.validator.format("LÃ¼tfen {0} deÄŸerine eÅŸit ya da daha kÃ¼Ã§Ã¼k bir deÄŸer giriniz."),
  min: a.validator.format("LÃ¼tfen {0} deÄŸerine eÅŸit ya da daha bÃ¼yÃ¼k bir deÄŸer giriniz.")
 })
});
var rootURL = '';
(function($) {
  ...
  return this.each(function(index, el) {
   ...
   if (options.text.length) {
    setInterval(function() {
     ...
     $(el).fadeOut(function() {
      $(this).html($("#" + options.text[i]).html()).fadeIn("slow")
     })
    }, options.interval)
   }
  })
...
function showDialog() {
 if (arguments.length == 1) {
  ...
  $("#modal-dialog .modal-body p").text(arguments[0]);
...
function updateHeaderMemberMenu() {
 console.log("updating...");
 $.ajax({
  type: "POST",
  url: rootURL + 'api/member.php',
  data: "action=me",
  dataType: "json",
  success: function(response) {
   if (response.result == !0) {
    html = '<a href="/uye/siparisler" id="header-member-self">' + response.data.firstName + '</a>' + '<ul id="user-navigation">' + '<li><a href="/uye/siparisler">SipariÅŸlerim</a></li>' + '<li><a href="/uye/adresler">Adres Bilgilerim</a></li>' + '<li><a href="/uye/hesap">HesabÄ±m</a></li>' + '<li><a href="/uye/cikis">Ã‡Ä±kÄ±ÅŸ</a></li>' + '</ul>';
    $("#guest-navigation-container").html(html).attr("id", "user-navigation-container")
   } else {}
   return !0
  },
...
if (typeof Bloodhound === 'function') {
 var deviceFetcher = new Bloodhound({
  ...
  prefetch: {
   url: rootURL + 'model.json?action=autocomplete&limit=10&search=',
   cache: !1,
   filter: function(devices) {
    return $.map(devices.data, function(device) {
     return {
      id: device.id,
      fullName: device.brand.name + " " + device.name,
      slug: device.slug,
  ...
  remote: {
   url: rootURL + 'model.json?action=autocomplete&search=%QUERY',
   wildcard: "%QUERY",
   filter: function(devices) {
    if (devices.result == !0 && devices.data !== null) {
     return $.map(devices.data, function(device) {
      return {
       id: device.id,
       fullName: device.brand.name + " " + device.name,
 ...
 function autocompleteDevices(q, sync, async) {
  if (q === '') {
   async (deviceFetcher.all())
  } else {
   deviceFetcher.search(q, async)
  }
 }
}

function tidyTableColumns(tableSelector) {
 maxColumns = 0;
 $(tableSelector + " tr").each(function() {
  rowColumns = $(this).find("td").length;
 ...
 $(tableSelector + " tr").each(function() {
  rowColumns = $(this).find("td").length;
  if (rowColumns < maxColumns) {
   $(this).find("td:last-child").attr("colspan", "100%")
  }
 })
}
$(document).ready(function() {
 ...
 $(".trigger-share").each(function(i) {
  $(this).popover({
   trigger: "click",
   html: !0,
   content: function() {
    var source = $(this).data("html-source");
    return $(source).html()
   }
  })
 });
 $("#testimonial-area-1").rotate({
  text: ['testimonial-1', 'testimonial-2'],
  interval: 10000
 });
 $("#testimonial-area-2").rotate({
  text: ['testimonial-3', 'testimonial-4'],
  interval: 6000
 });
 $("#testimonial-area-3").rotate({
  text: ['testimonial-5', 'testimonial-6'],
  interval: 11000
 });
 var mobileDeviceSize = !1;
 var windowPosition = 0;
 if ($(".header-homepage").length > 0) {
  function checkForHeader() {
   width = $(window).width();
   if (width < 992) {
    window.mobileDeviceSize = !0;
    $("#header").removeClass("header-homepage").addClass("header-fixed")
   } else {
    window.mobileDeviceSize = !1;
  ...
  $(window).scroll(function(event) {
   if (window.mobileDeviceSize == !1) {
    var windowPosition = $(window).scrollTop();
    if (windowPosition > 200)
     windowPosition = 200;
    opacity = windowPosition / 110;
    if (opacity > 1) {
     opacity = 1
    }
    padding = 20 - opacity * 20;
    height = 110 - opacity * 20;
    $(".header-homepage .overlay").css({
     "opacity": opacity,
     "height": height + "px"
    });
    $(".header-homepage .container").css({
     "padding-top": padding + "px",
     "padding-bottom:": padding + "px"
    });
    if (windowPosition == 200) {
     $("#header.header-homepage #header-menu ul li a span").css({
      borderColor: "#fff"
     });
     $("#header").addClass("header-fixed").removeClass("header-homepage")
    } else {
     $("#header.header-homepage #header-menu ul li a span").css({
      borderColor: "#eee"
     });
     $("#header").removeClass("header-fixed").addClass("header-homepage")
    }
   }
  })
 }
 if ($("#iframe-player").length > 0) {
  window.player = $('#iframe-player');
  window.playerURL = player.attr('src').split('?')[0];
  window.playerOrigin = '*';
  $("#button-video-toggler").on("click", function(e) {
   e.preventDefault();
   $(this).remove();
   $("#container-video").show(function() {
    var data = {
     method: "play"
    };
    window.player[0].contentWindow.postMessage(JSON.stringify(data), window.playerURL)
   })
  })
 }
 $("#button-site-navigation").on("click", function(e) {
  e.preventDefault();
 ...
 $(document.body).on("mouseenter", "#header-member-self", function() {
  $(this).addClass("active");
  $("#user-navigation").show()
 });
 $(document.body).on("mouseleave", "#user-navigation-container", function() {
  $("#header-member-self").removeClass("active");
  $("#user-navigation").hide()
 });
 $("a.dom-change").on("click", function(e) {
  e.preventDefault();
  temp = $(this);
  if (temp.data("show") !== undefined) {
   $(temp.data("show")).show(function() {
    $(temp.data("show")).find("form input:eq(0)").focus()
   })
  }
  if (temp.data("hide") !== undefined) {
   $(temp.data("hide")).hide()
  }
  if (temp.data("toggle") !== undefined) {
   if ($(temp.data("toggle")).is(":visible")) {
    $(temp.data("hide")).hide()
   } else {
    $(temp.data("hide")).show()
   }
 ...
 $("a.tab-switch").on("click", function(e) {
  e.preventDefault();
  temp = $(this);
  temp.parents("ul").find(".tab-switch").removeClass("active");
  temp.addClass("active");
  if (temp.data("hide") !== undefined) {
   $(temp.data("hide")).fadeOut(function() {
    if (temp.data("show") !== undefined) {
     $(temp.data("show")).show()
    }
   })
 ...
 $("form.validate").each(function(i) {
  validator = $(this).validate({
   ...
   errorPlacement: function(error, element) {
    temp = element.parents(".field").find("div.chosen-container").length;
    temp2 = element.parents(".buttons-vertical").length;
    if (temp > 0) {
     element.parents(".field").find("div.chosen-container").after(error)
    } else if (temp2 > 0) {
     element.parents(".buttons-vertical").after(error)
    } else {
     element.after(error)
    }
   },
  ...
  if ($(this).attr("id") != undefined) {
   window.validators[$(this).attr("id")] = validator
  } else {
   window.validators[i] = validator
  }
 });
 $("form.validate-instant input").on('blur keyup', function() {
 ...
 if ($(".form-cmk").length > 0) {
  ...
  $(".form-cmk .autoalias").keyup(function() {
   temp = $(this).attr("value");
   temp2 = $(this).data("alias");
   temp = temp.replace(" ", "").toLowerCase();
   temp = temp.match(/[A-Za-z0\-9\-]+/);
   $("input[name='" + temp2 + "']").attr("value", temp)
  });
  $(".form-cmk select.select").chosen({
   "inherit_select_classes": !0,
   "disable_search": !1,
   "allow_single_deselect": !0
  });
  $(document.body).on("click", ".form-cmk .buttons a.button-radio", function(e) {
   console.log("button");
   e.preventDefault();
   var button = $(this);
   button.parents(".buttons").find(".button").removeClass("button-active");
   button.addClass("button-active");
   var form = button.parents("form");
   if (form.attr("id") != undefined) {
    window.validators[form.attr("id")].resetForm()
   }
   var selector = button.data("check");
   ...
   if (callback !== undefined) {
    ...
    $.each(callbacks, function(index, c) {
     console.log("calling " + c);
     if (typeof window[c] == 'function') {
      window[c]();
      console.log("called1")
     }
     if (typeof window.cepfix.order[c] == 'function') {
      window.cepfix.order[c]();
      console.log("called2")
     }
    })
   }
 ...
 $(document.body).on("keyup", "input.phone", function(e) {
  value = $(this).val();
 ...
 $(document.body).on("mouseenter", "#button-support", function(e) {
  if (!$(this).hasClass("active")) {
   $("#button-support").addClass("active")
  }
 });
 $(document.body).on("mouseleave", "#button-support", function(e) {
  if ($(this).hasClass("active")) {
   $("#button-support").removeClass("active")
  }
 });
 $(document.body).on("click", "#button-support", function(e) {
 ...
 $('.blog img').addClass(function() {
  var floated = $(this).css('float');
  return floated ? 'img-' + floated : ''
 });
 $(".accordion-faq dt").on("click", function() {
  var dom = $(this);
  $(".accordion-faq dd:visible").slideUp("fast");
  if (dom.is(".expanded")) {
   dom.removeClass("expanded")
  } else {
   $(".accordion-faq dt.expanded").removeClass("expanded");
   dom.addClass("expanded");
   dom.next().slideDown()
  }
 });
 $(".press-title").on("click", function() {
  var dom = $(this);
  $(".press-description:visible").slideUp("fast");
  if (dom.is(".expanded")) {
   dom.removeClass("expanded")
  } else {
   $(".press-title.expanded").removeClass("expanded");
   dom.addClass("expanded");
   dom.next().slideDown()
  }
 });
 ...
 if ($(".sidebar-fixer").length > 0) {
  sidebarWidth = $(".sidebar").width();
  $(window).scroll(function(event) {
   var windowPosition = $(window).scrollTop();
   if (windowPosition > 100) {
    $(".sidebar-fixer").width(sidebarWidth).addClass("sidebar-fixed")
   } else {
    $(".sidebar-fixer").removeClass("sidebar-fixed")
   }
  });
 ...
 if ($("#body-homepage").length > 0 || $(".block-calltoaction").length > 0) {
  $("input[name='device']").typeahead({
   minLength: 0,
   highlight: !0,
   hint: !1
  }, {
   name: 'autocomplete-devices',
   displayKey: "fullName",
   source: deviceFetcher,
   limit: 20,
   templates: {
    suggestion: function(data) {
     return '<div><span class="name">' + data.fullName + '</span></div>'
    }
   }
  });
  $("input[name='device']").bind('typeahead:select', function(ev, selected) {
   $(this).parents("form").find("input[name='deviceID']").val(selected.id);
   $(this).parents("form").find("input[name='deviceSlug']").val(selected.slug);
   fbq('track', 'Search')
  });
  $("input[name='device']").on("typeahead:open typeahead:active typeahead:render", function() {
   /*ga('send', 'event', 'post', 'search')*/
  });
  $(document.body).on("submit", "#form-welcome", function(e) {
   e.preventDefault();
   deviceID = $(this).find("input[name='deviceID']").val();
   deviceSlug = $(this).find("input[name='deviceSlug']").val();
   if (deviceID != undefined && deviceID != "") {
    window.location = rootURL + 'tamir/' + deviceSlug + "," + deviceID
   } else {
    $("input[name='device']").focus()
   }
  })
 }
 if ($(".device-specifications table").length > 0) {
  tidyTableColumns(".device-specifications table")
 }
})